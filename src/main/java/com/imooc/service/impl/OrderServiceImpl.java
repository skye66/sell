...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderDTO findOne(String oderId) {
        OrderMaster orderMaster = orderMasterRepository.findById(oderId).get();
        if (orderMaster == null){
    ...
    public OrderDTO cancel(OrderDTO orderDTO) {
        ...
        if (!orderDTO.getOrderStatus().equals(OrderStatusEnum.NEW.getCode())){
            log.error("【取消订单】订单状态正确，orderId={}, orderStatus={}", orderDTO.getOrderId(), orderDTO.getOrderStatus());
    ...
    public OrderDTO finished(OrderDTO orderDTO) {
        if (!orderDTO.getOrderStatus().equals(OrderStatusEnum.NEW.getCode())){
            log.error("【完结订单】完结订单失败 orderId = {}, orderStatus = {}",orderDTO.getOrderId(), orderDTO.getOrderStatus());
            throw new SellException(ResultEnum.ORDER_STATUS_ERROR);
        }
        OrderMaster orderMaster = new OrderMaster();
    ...
    public OrderDTO paid(OrderDTO orderDTO) {
        ...
        if (!orderDTO.getPayStatus().equals(PayStatusEnum.WAIT.getCode())) {
            log.error("【支付订单】订单支付状态不正常 orderDTO = {}", orderDTO);
        ...
        OrderMaster resultUpdate = orderMasterRepository.save(orderMaster);
        if (resultUpdate == null){
            log.error("【支付订单】更新，orderMaster={}", orderMaster);
            throw new SellException(ResultEnum.ORDER_UPDATE_FAIL);
        }
        //推送微信模版消息
